...
do
    cat $i | python3 -c 'import json, sys; from ruamel import yaml ; y=yaml.safe_load(sys.stdin.read()) ; json.dump(y, sys.stdout)' > workflow-lint-$i.json
    echo "Validating GitHub workflow $i"
    jsonschema -i workflow-lint-$i.json github-workflow.json
done
